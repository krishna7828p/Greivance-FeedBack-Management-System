...
function ResetPassword(props) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const temp = { ...pass };
    temp.oldPassword = pass.oldPassword;
    temp.newPassword = pass.newPassword;
    temp.confirmPassword = pass.confirmPassword;
    setPass({ ...temp });
    ...
    if (
      ...
      changePassword(JSON.parse(localStorage.getItem("user")).empId, temp).then(
        (resp) => {
          localStorage.clear();
          navigate("/");
        }
